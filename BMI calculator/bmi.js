function bmiCal(){
    ...
    if (isNaN(mass) || isNaN(height) || mass <= 0 || height <= 0) {
        document.getElementById('bmi-msg').innerHTML = 'Please enter valid weight and height.';
        return;
    }
    let bmi = mass/(heightM**2);
    let result=bmi.toFixed(1);
    result_msg.innerText=`BMI = ${result}`;
    healthyWeight.innerText=`Healthy weight for the height: ${calculateHealthyWeightRange(height)}`
